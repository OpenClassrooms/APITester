...
namespace OpenAPITesting\Symfony\Component\PropertyInfo;
...
class Type
{
    ...
    public function __construct(
        ...
        $this->collectionValueType = $this->validateCollectionArgument(
                $collectionValueType,
                6,
                '$collectionValueType'
            ) ?? [];
    ...
    private function validateCollectionArgument($collectionArgument, int $argumentIndex, string $argumentName): ?array
    {
        ...
        if (!\is_array($collectionArgument) && !$collectionArgument instanceof self) {
            throw new \TypeError(
                sprintf(
                    '"%s()": Argument #%d (%s) must be of type "%s[]", "%s" or "null", "%s" given.',
                    __METHOD__,
                    $argumentIndex,
                    $argumentName,
                    self::class,
                    self::class,
                    get_debug_type($collectionArgument)
                )
            );
        }
        ...
        if (\is_array($collectionArgument)) {
            foreach ($collectionArgument as $type) {
                if (!$type instanceof self) {
                    throw new \TypeError(
                        sprintf(
                            '"%s()": Argument #%d (%s) must be of type "%s[]", "%s" or "null", array value "%s" given.',
                            __METHOD__,
                            $argumentIndex,
                            $argumentName,
                            self::class,
    ...
    public function getCollectionKeyType(): ?self
    {
        trigger_deprecation(
            ...
            '5.3',
            'The "%s()" method is deprecated, use "getCollectionKeyTypes()" instead.',
            __METHOD__
        );
    ...
    public function getCollectionValueType(): ?self
    {
        trigger_deprecation(
            'symfony/property-info',
            '5.3',
            'The "%s()" method is deprecated, use "getCollectionValueTypes()" instead.',
            __METHOD__
        );
